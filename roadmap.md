...
| 系統 | 說明 | 玩法 |
|------|------|------|
| AI 角色扮演 | 薇拉 (Vera)，18 歲女高生 / 酒館兼職 | 直接打字對話，由 DeepSeek V4 Pro 驅動 |
...
| `doggy` | 後入（陰道，與 anal 區分） | 「汪系僕從：後入式 20 次」 |
...
> **保守建議**：先增加體位類（doggy, cowgirl, 69, deepthroat），情境類挑選 3-4 個最符合薇拉人設的（school_uniform, pantyhose, blindfold, shower）。全部追蹤太過臃腫，其他留作 AI 情境文本即可。
...
在 `users` 表加入 `mood` 欄位（`TEXT`，枚舉值），影響薇拉回覆語氣：
...
禮物商店改為用金幣購買：
- 巧克力 → 10 金幣 (對薇拉回覆 reply:「/coin send 10」)
- 玫瑰 → 30 金幣 (對薇拉回覆 reply:「/coin send 30」)
...
**目標**：將薇拉由「被動聊天機器人」進化為「有記憶、有主見的群組成員」
...
*本文件由 DeepSeek TUI 生成，作為 Vera Bot 的長期發展藍圖。*
