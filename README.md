...
- **動態關係評估系統**：不再使用生硬的數值好感度。Vera 會根據歷史記憶與客人的發言摘要，自主判斷該對此人展現「智力上的尊重」或是「冷淡的不屑」。
...
### 5. 🏷️ 動態標籤與稱號系統 (Dynamic Titles)
- **行為分析**：Vera 會在後台觀察每位客人的發言習慣與互動內容。
- **自動賦予特徵**：當累積足夠的數據後，AI 會自動為該客人生成專屬的「行為特徵標籤」（例如：`深夜話癆`、`貓科動物觀測對象`），並保存在資料庫中。
- **個人面板檢視**：客人可透過 `/profile` 指令查看自己目前的行為特徵標籤與觀測摘要。
...
- `/profile` - 檢視妳的個人觀測檔案（包含特徵標籤與觀測摘要）。
